...
# ******************** メインループ ********************
def main():
    global auto_play_mode #自動プレイモード
    auto_play_mode = False # デフォルトは自動プレイモードをオフにする
    ...
    def auto_play():
        ...
        # 選択された方向に偽のキー入力を生成
        fake_key_input = {key: False for key in directions}
        fake_key_input[selected_direction] = True
        return fake_key_input

    # ゲームのメインループ内でオートプレイ機能を使用
while True:
    # オートプレイが有効な場合、ランダムな方向に移動
    if auto_play_mode:  # auto_play_modeはオートプレイを制御するためのフラグです
        key = auto_play()
    else:
        key = pygame.key.get_pressed()
    
    move_player(key)  # プレイヤーの移動処理をキー入力に基づいて実行




 # ゲームプレイデータを記録する関数
    def log_gameplay_data(action, state, reward, done):
        with open('gameplay_data.json', 'a') as file:
            data = {'action': action, 'state': state, 'reward': reward, 'done': done}
            json.dump(data, file)
            file.write('\n')
    ...
    while True:
        ...
        # ゲームプレイ
            ...
            # プレイ中
                ...
                current_state = {
                    'player_position': {'x': pl_x, 'y': pl_y},
                    'enemies_positions': [{'x': emy_x[i], 'y': emy_y[i], 'active': emy_f[i]} for i in range(len(emy_f)) if emy_f[i]],
                    'coins_collected': pl_coin,
                    'lives_left': pl_life,  #　プレイヤーのライフ
                }
                move_player(key)    # プレイヤーの動き
                log_gameplay_data('move_player', current_state, 0, False)
                move_enemy()    # 敵の動き
                # log_gameplay_data('move_enemy', current_state, 0, False)
                use_item(key)       # アイテムの使用
                # log_gameplay_data('use_item', current_state, 0, False)
                hit_check()         # ヒットチェック
                # log_gameplay_data('hit_check', current_state, 0, False)
